...
presenceIsland <- simulateExtinctions_multipleIterations(presenceMainland, presenceIsland, 5)
...
#Generate Data
connectanceControlValues <- c(0.05)
pControlValues <- c(0.1)
for(connectance in connectanceControlValues){
  for(prob in pControlValues){
    pop_data <- list()
    pop_data[[1]] <- c(1:300)
    superMainlandPopIDs <- list()
    for(superIndex in 1:3){
      ...
      IndividualMainlandPopIDs <- list()
      for(independentRun in 1:10){
        mainlandPopIDs <- list()
        presenceIsland <- list()
        for(i in 1:length(stage_list)){
          if(is.null(dim(stage_list[[i]]))){
        ...
        for(superIndex2 in 1:300){
          presenceIsland <- migrate(presenceMainland, presenceIsland)
          presenceIsland <- simulateExtinctions_multipleIterations(presenceMainland, presenceIsland, 1)
          mainlandPopIDs[[length(mainlandPopIDs) + 1]] <- getActiveSpeciesIDs(presenceIsland)
        }
        IndividualMainlandPopIDs[[length(IndividualMainlandPopIDs) + 1]] <- mainlandPopIDs
      }
      superMainlandPopIDs[[length(superMainlandPopIDs) + 1]] <- IndividualMainlandPopIDs
    }
  }
}
mainlandA <- superMainlandPopIDs[[1]]
mainlandB <- superMainlandPopIDs[[2]]
mainlandC <- superMainlandPopIDs[[3]]

gammaValuesMainlandA <- c()
gammaValuesMainlandB <- c()
gammaValuesMainlandC <- c()

alphaValuesMainlandA <- list()
alphaValuesMainlandB <- list()
alphaValuesMainlandC <- list()

betaValuesMainlandA <- c()
betaValuesMainlandB <- c()
betaValuesMainlandC <- c()


#filling missing values in mainlandA/B/C
for(i in 1:length(mainlandA)){
  currentLength <- length(mainlandA[[i]])
  if(currentLength < 300){
    diff = 300 - currentLength
    lead <- as.list(rep(0, diff))
    mainlandA[[i]] <- c(lead, mainlandA[[i]])
  }
}

for(i in 1:length(mainlandB)){
  currentLength <- length(mainlandB[[i]])
  if(currentLength < 300){
    diff = 300 - currentLength
    lead <- as.list(rep(0, diff))
    mainlandB[[i]] <- c(lead, mainlandB[[i]])
  }
}

for(i in 1:length(mainlandC)){
  currentLength <- length(mainlandC[[i]])
  if(currentLength < 300){
    diff = 300 - currentLength
    lead <- as.list(rep(0, diff))
    mainlandC[[i]] <- c(lead, mainlandC[[i]])
  }
}
...
for(i in 1:length(mainlandA)){
  alphaForIsland <- c()
  for(j in 1:length(mainlandA[[i]])){
    alphaForIsland[length(alphaForIsland) + 1] <- length(unique(mainlandA[[i]][[j]]))
  }
  alphaValuesMainlandA[[length(alphaValuesMainlandA) + 1]] <- alphaForIsland
}

for(i in 1:length(mainlandB)){
  alphaForIsland <- c()
  for(j in 1:length(mainlandB[[i]])){
    alphaForIsland[length(alphaForIsland) + 1] <- length(unique(mainlandB[[i]][[j]]))
  }
  alphaValuesMainlandB[[length(alphaValuesMainlandB) + 1]] <- alphaForIsland
}

for(i in 1:length(mainlandC)){
  alphaForIsland <- c()
  for(j in 1:length(mainlandC[[i]])){
    alphaForIsland[length(alphaForIsland) + 1] <- length(unique(mainlandC[[i]][[j]]))
  }
  alphaValuesMainlandC[[length(alphaValuesMainlandC) + 1]] <- alphaForIsland
}

#Calculating Gamma values
superSpeciesSetA <- list()
for(i in 1:300){
  for(j in 1:length(mainlandA)){
    if(length(superSpeciesSetA) < i){
      superSpeciesSetA[[i]] <- mainlandA[[j]][[i]]
    }
    else{
      superSpeciesSetA[[i]] <- c(superSpeciesSetA[[i]], mainlandA[[j]][[i]])
    }
  }
}
for(i in 1:length(superSpeciesSetA)){
  gammaValuesMainlandA[i] <- length(unique(superSpeciesSetA[[i]]))
}

superSpeciesSetB <- list()
for(i in 1:300){
  for(j in 1:length(mainlandB)){
    if(length(superSpeciesSetB) < i){
      superSpeciesSetB[[i]] <- mainlandB[[j]][[i]]
    }
    else{
      superSpeciesSetB[[i]] <- c(superSpeciesSetB[[i]], mainlandB[[j]][[i]])
    }
  }
}
for(i in 1:length(superSpeciesSetB)){
  gammaValuesMainlandB[i] <- length(unique(superSpeciesSetB[[i]]))
}

superSpeciesSetC <- list()
for(i in 1:300){
  for(j in 1:length(mainlandC)){
    if(length(superSpeciesSetC) < i){
      superSpeciesSetC[[i]] <- mainlandC[[j]][[i]]
    }
    else{
      superSpeciesSetC[[i]] <- c(superSpeciesSetC[[i]], mainlandC[[j]][[i]])
    }
  }
}
for(i in 1:length(superSpeciesSetC)){
  gammaValuesMainlandC[i] <- length(unique(superSpeciesSetC[[i]]))
}

#Calculating Beta values
transformedAlphaA <- list()
for(i in 1:300){
  for(j in 1:length(alphaValuesMainlandA))
    if(length(transformedAlphaA) < i){
      transformedAlphaA[[i]] <- alphaValuesMainlandA[[j]][[i]]
    }
    else{
      transformedAlphaA[[i]] <- c(transformedAlphaA[[i]], alphaValuesMainlandA[[j]][[i]]) 
    }
}

transformedAlphaMeansA <- c()
for(i in 1:length(transformedAlphaA)){
  transformedAlphaMeansA[i] <- mean(transformedAlphaA[[i]])
}
for(i in 1:300){
  betaValuesMainlandA[i] <- gammaValuesMainlandA[i]/transformedAlphaMeansA[i]
}


transformedAlphaB <- list()
for(i in 1:300){
  for(j in 1:length(alphaValuesMainlandB))
    if(length(transformedAlphaB) < i){
      transformedAlphaB[[i]] <- alphaValuesMainlandB[[j]][[i]]
    }
    else{
    transformedAlphaB[[i]] <- c(transformedAlphaB[[i]], alphaValuesMainlandB[[j]][[i]]) 
  }
}

transformedAlphaMeansB <- c()
for(i in 1:length(transformedAlphaB)){
  transformedAlphaMeansB[i] <- mean(transformedAlphaB[[i]])
}
for(i in 1:300){
  betaValuesMainlandB[i] <- gammaValuesMainlandB[i]/transformedAlphaMeansB[i]
}

transformedAlphaC <- list()
for(i in 1:300){
  for(j in 1:length(alphaValuesMainlandC))
    if(length(transformedAlphaC) < i){
      transformedAlphaC[[i]] <- alphaValuesMainlandC[[j]][[i]]
    }
    else{
      transformedAlphaC[[i]] <- c(transformedAlphaC[[i]], alphaValuesMainlandC[[j]][[i]]) 
    }
}

transformedAlphaMeansC <- c()
for(i in 1:length(transformedAlphaB)){
  transformedAlphaMeansC[i] <- mean(transformedAlphaC[[i]])
}
for(i in 1:300){
  betaValuesMainlandC[i] <- gammaValuesMainlandC[i]/transformedAlphaMeansC[i]
}

diversityData <- list()
diversityData[[1]] <- c(1:300)
diversityData[[2]] <- gammaValuesMainlandA
diversityData[[3]] <- gammaValuesMainlandB
diversityData[[4]] <- gammaValuesMainlandC

fileName <- "C:/Users/Akhil/Documents/Notes Bio Research/Repos/food_webs/diversityData_C0.05_p0.1.csv"
print(fileName)
write.table(diversityData, file = fileName,row.names=FALSE, na="",col.names=FALSE, sep=",")
